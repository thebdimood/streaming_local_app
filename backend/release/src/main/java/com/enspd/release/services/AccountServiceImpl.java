...
import com.enspd.release.Exceptions.WrongPasswordException;
...
public class AccountServiceImpl implements AccountService {
    ...
    public AccountRequestDTO AccountInfo(Integer id) {
        AccountEntity accountEntity = accountRepository.findById(id).orElseThrow(
                () -> new RessourceNotFoundException("the user with id " + id + " not found"));
    ...
    /*
     * cette operation permet la mise a jour des informations du compte hormis le
     * mots de passe
     * 
     * @param required id
     * 
    ...
    public AccountRequestDTO UpdateInfo(AccountUpdateRequestDTO accountUpdateRequestDTO) {
        ...
        AccountEntity accountEntity = accountRepository.findById(accountUpdateRequestDTO.getId()).orElseThrow(
                () -> new RessourceNotFoundException("the user with id " + accountRequestDTO.getId() + " not found"));
    ...
    /*
     * cette operation permet de mettre a jour le mot de passe d'un compte en
     * particulier
     * 
     * @required param :id
     * 
    ...
    public void ChangePassword(PasswordChangeorUpdate passwordChangeorUpdate) {
        ...
        AccountEntity accountEntity = accountRepository.findById(passwordChangeorUpdate.getId()).orElseThrow(
                () -> new RessourceNotFoundException(
                        "the user with id " + passwordChangeorUpdate.getId() + " not found"));
    ...
    /*
     * cette operation permet davoir la liste des comptes sur la plateforme sans
     * aucun filtrage a faire avec beaucoup de precaution
    ...
    public void DeleteAccount(AccountDeleteDTO accountDeleteDTO) {
        AccountEntity accountEntity = accountRepository.findById(accountDeleteDTO.getId()).orElseThrow(
                () -> new RessourceNotFoundException("the user with id " + accountDeleteDTO.getId() + " not found"));
